...
dGeo = [ 
    { label: "Traffic Analysis Zones", value: "TAZ"      , defaultLineWidth: 0.4, idfieldname: "TAZID"    , idfieldtype: "number", chartfileprefix: "TAZ"      , charttitletext: "TAZ "            , legendtitletext: "TAZ"     , roundchartvalues:1, minScaleForLabels: 87804  },
    { label: "City Areas"            , value: "CITYAREA" , defaultLineWidth: 1.5, idfieldname: "CityArea" , idfieldtype: "string", chartfileprefix: "CityArea" , charttitletext: ""                , legendtitletext: "City"    , roundchartvalues:1, minScaleForLabels: 1000000},
    { label: "Medium Districts"      , value: "DISTMED"  , defaultLineWidth: 1.5, idfieldname: "DISTMED"  , idfieldtype: "number", chartfileprefix: "DISTMED"  , charttitletext: "Medium District ", legendtitletext: "District", roundchartvalues:1, minScaleForLabels: 1000000},
    { label: "County"                , value: "COUNTY"   , defaultLineWidth: 5.0, idfieldname: "CO_NAME"  , idfieldtype: "string", chartfileprefix: "CO_NAME"  , charttitletext: ""                , legendtitletext: "County"  , roundchartvalues:1, minScaleForLabels: 2000000},
    { label: "Other Areas"           , value: "OTHERAREA", defaultLineWidth: 1.5, idfieldname: "OtherArea", idfieldtype: "string", chartfileprefix: "OtherArea", charttitletext: ""                , legendtitletext: "Area"    , roundchartvalues:1, minScaleForLabels: 1000000}
];
...
define(['dojo/_base/declare',
    ...
    'dojo/domReady!'],
function(declare, BaseWidget, LayerInfos, RainbowVis, registry, dom, domStyle, djQuery, dijit, Chart, Claro, Julie, SimpleTheme, Scatter, Lines, Columns, Legend, SelectableLegend, Tooltip, TableContainer, ScrollPane, ContentPane, PanelManager, TextBox, Query, QueryTask, FeatureLayer, FeatureTable, SimpleFillSymbol, SimpleLineSymbol, SimpleMarkerSymbol, TextSymbol, Font, LabelClass, InfoTemplate, Color, Map, SimpleRenderer, UniqueValueRenderer, Extent, Memory, StoreSeries, Dialog, Button, RadioButton, MutliSelect, CheckedMultiSelect, Select, ComboBox, CheckBox, HorizontalSlider, Observable, cookie, esriLang, jimuUtils) {
...
startup: function() {
    ...
    var _cmbGeos = new Select({
        ...
        onChange: function(){
            ...
            wF._closeFeedback();
            wF.map.infoWindow.clearFeatures();
            wF._updateDisplay();
        }
    }, "cmbGeo");
    _cmbGeos.startup();
    ...
    wF.map.on('click', selectFeatures);    
    
  
    function pointToExtent(map, point, toleranceInPixel) {  
        var pixelWidth = wF.map.extent.getWidth() / wF.map.width;  
        var toleranceInMapCoords = toleranceInPixel * pixelWidth;  
        return new Extent(point.x - toleranceInMapCoords,  
            point.y - toleranceInMapCoords,  
            point.x + toleranceInMapCoords,  
            point.y + toleranceInMapCoords,  
            wF.map.spatialReference);  
    }
    ...
    function selectFeatures(evt) {
        ...
        function showResults(results) {
            ...
            if (resultCount>=1) {
                ...
                wF.publishData({message: "update feedback"});
                ...
                //wF.map.infoWindow.lineSymbol = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, Color.fromHex(sSelectionColor), 3);
                wF.map.infoWindow.fillSymbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_NULL, new SimpleLineSymbol(SimpleLineSymbol.STYLE_DASHDOT, new Color([255,255,0]), 3), new Color([255,255,0,0.25]));
                wF.map.infoWindow.setFeatures([_selectFeature]);
    ...
    var anim_a = new Tooltip(cChart     , "default");
    var anim_a = new Tooltip(cChartBasic, "default");
...
_updateDisplay: function() {
    ...
    if (typeof classbreaks !== 'undefined') {
        ...
        var _classbreaks = classbreaks.filter( function(classbreaks){return (classbreaks.geography==curGeo && classbreaks.categoryCode==curCat && classbreaks.metric==curMet && classbreaks.forecastorchange==_forecastorchange && classbreaks.absoluteorpercent==_absoluteorpercent);} )[0];
        ...
        if (typeof _classbreaks !== 'undefined') {
            ...
            for (var l=_classbreaks.numClasses-1; l>=0; l--) {
                ...
                if (l==_classbreaks.numClasses-1) { // last class 
                    var _codeblock =  "var displayvalue = " + _scoreExp + "; " +
    ...
    var _line         = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, Color.fromHex("#888888"), 1.5);
    var _line_outline = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, Color.fromHex("#FFFFFF"), 3.5);
...
_buildTable: function() {
    ...
    if (curTab=='FORECAST') {
      wF._insertSorted(_yearsonly,curYear,100);
    } else if (curTab=='CHANGE') {
      wF._insertSorted(_yearsonly,curYearFrom,100);
      wF._insertSorted(_yearsonly,curYearTo,100);
    } else if (curTab=='NEWVSOLD') {
      wF._insertSorted(_yearsonly,curYearNewOld,100);
    }
    ...
    for (y in _years) {
        ...
        for (c in dCat) {
            ...
            // append cell HTML
            _strDataRowsHMTL_New += '<td align="right" width="12.5%"' + _sHighlighText + '>' + _sPrefix + wF._NumberWithCommas(_value_New) + _sSuffix + '</td>';
            if (curTab=='NEWVSOLD') { // only  highlight Forecast 2019 Values  if NEWVSOLD
              _strDataRowsHMTL_Old += '<td align="right" width="12.5%"' + _sHighlighText + '>' + _sPrefix + wF._NumberWithCommas(_value_Old) + _sSuffix + '</td>';
            } else { // 
              _strDataRowsHMTL_Old += '<td align="right" width="12.5%">' + _sPrefix + wF._NumberWithCommas(_value_Old) + _sSuffix + '</td>';
            }
        }
        // append row end HTML
...
_incrementDown: function() {
  ...
  if (curTab=='FORECAST') {
    ...
    if (parseInt(dom.byId('cmbInterval').value)!=1) {
      _newYear = wF._roundNearest5(_newYear);
    }
    ...
    if (parseInt(dom.byId('cmbInterval').value)!=1) {
      ...
      _newYearFrom = wF._roundNearest5(_newYearFrom);
    }
    ...
    if (parseInt(dom.byId('cmbInterval').value)!=1) {
      ...
      _newYearTo = wF._roundNearest5(_newYearTo);
    }
    ...
    if (parseInt(dom.byId('cmbInterval').value)!=1) {
      _newYearNewOld = wF._roundNearest5(_newYearNewOld);
    }
    ...
    function showResults(featureSet) {
      ...
      if (featureSet.features.length>0) {
        ...
        for (i = 0; i < featureSet.features.length; i++) { 
          ...
          wF.map.graphics.add(graphic); 
        } 

        wF.map.setExtent(thisExtent.expand(2)); 
